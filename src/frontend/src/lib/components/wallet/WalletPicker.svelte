<script lang="ts">
	...
	import { missionControlIcpBalanceOrZero } from '$lib/derived/wallet/balance.derived';
...
<div>
	<Value ref="wallet-id">
		...
		<select id="wallet-id" name="wallet-id" bind:value={walletIdText}>
			...
			{#if nonNullish($missionControlId) && (!filterMissionControlZeroBalance || $missionControlIcpBalanceOrZero > 0n)}<option
