...
export const getLocalStorageAnalyticsPeriodicity = (): AnalyticsPeriodicity => {
	try {
		const { analytics_periodicity }: Storage = browser
			? localStorage
			: ({ analytics_periodicity: JSON.stringify(DEFAULT_ANALYTICS_PERIODICITY) } as unknown as Storage);

		return nonNullish(analytics_periodicity) ? JSON.parse(analytics_periodicity) : DEFAULT_ANALYTICS_PERIODICITY;
	} catch (err: unknown) {
		// We use the local storage for the operational part of the app but, not crucial
		console.error(err);
