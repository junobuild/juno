...
where
    I: Iterator<Item = (StableKey, Doc)> + 'a,
{
    let matches = filter_owned_values(caller, controllers, &rule.read, docs, filters);

    let results = list_owned_values(matches, filters);
