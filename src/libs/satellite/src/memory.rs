...
const DB_USERS: MemoryId = MemoryId::new(4);
...
fn get_memory_db_users() -> Memory {
    MEMORY_MANAGER.with(|m| m.borrow().get(DB_USERS))
}
...
pub fn init_stable_state() -> StableState {
    StableState {
        ...
        db_users: StableBTreeMap::init(get_memory_db_users()),
    }
}
