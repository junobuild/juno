...
use crate::storage::constants::{ASSET_ENCODING_NO_COMPRESSION, BN_WELL_KNOWN_CUSTOM_DOMAINS, WELL_KNOWN_II_ALTERNATIVE_ORIGINS};
use crate::storage::http::types::HeaderField;
use crate::storage::runtime::{delete_certified_asset, update_certified_asset as update_runtime_certified_asset};
use crate::storage::state::{delete_asset, get_asset as get_state_asset, get_domains, get_rule, insert_asset as insert_state_asset};
...
pub fn update_alternative_origins_asset(alternative_origins: &String) -> Result<(), String> {
    ...
    update_asset(&full_path, alternative_origins, &map_alternative_origins_asset)
}
...
pub fn update_custom_domains_asset() -> Result<(), String> {
    ...
    update_asset(&full_path, &custom_domains, &map_custom_domains_asset)
}
...
fn update_asset(full_path: &String, content: &String, f: &dyn Fn(&String, Option<Asset>) -> Asset) -> Result<(), String> {
    let collection = DEFAULT_ASSETS_COLLECTIONS[0].0.to_string();
