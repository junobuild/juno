...
pub fn delete_user_usage(user_id: &UserId) -> Result<(), String> {
    ...
    delete_user_usage_for_collections(user_id, &CollectionType::Storage, &user_usage_collection, &rule)?;

    Ok(())
}

fn delete_user_usage_for_collections(user_id: &UserId, collection_type: &CollectionType, user_usage_collection: &CollectionKey, user_usage_rule: &Rule) -> Result<(), String> {
