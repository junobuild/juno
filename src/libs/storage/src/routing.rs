...
pub fn get_routing(
    ...
    let alternative_asset = get_alternative_asset(&path, &token, content_store);
    ...
    if include_alternative_routing {
        ...
        let root_rewrite = get_routing_root_rewrite(&path, content_store);
...
fn get_routing_rewrite(
    ...
    match rewrite {
        ...
        Some(rewrite) => {
            ...
            let rewrite_asset = get_alternative_asset(&destination, token, content_store);
...
fn get_routing_root_rewrite(path: &FullPath, content_store: &impl ContentStore) -> Option<Routing> {
