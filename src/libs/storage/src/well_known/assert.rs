...
pub fn assert_not_well_known_keys(full_path: &FullPath) -> Result<(), String> {
    // /.well-known/ic-domains is automatically generated for custom domains
    assert_not_well_known_key(full_path, WELL_KNOWN_CUSTOM_DOMAINS)?;

    // /.well-known/ii-alternative-origins is automatically generated for alternative origins
    assert_not_well_known_key(full_path, WELL_KNOWN_II_ALTERNATIVE_ORIGINS)?;

    Ok(())
}

fn assert_not_well_known_key(full_path: &str, reserved_path: &str) -> Result<(), String> {
