...
use crate::cycles_monitoring::store::{disable_mission_control_monitoring, disable_orbiter_monitoring, disable_satellite_monitoring};
...
fn stop_mission_control_monitoring() -> Result<(), String> {
    RUNTIME_STATE
        .with(|state| stop_mission_control_monitoring_impl(&mut state.borrow_mut()))
}

fn stop_mission_control_monitoring_impl(
    state: &mut RuntimeState,
) -> Result<(), String> {
    let fund_manager = state.fund_manager.get_or_insert_with(init_funding_manager);
    ...
    if strategy_exists.len() > 1 {
        return Err("Mission control monitoring cannot be disabled while some modules remain active.".to_string());
    }
