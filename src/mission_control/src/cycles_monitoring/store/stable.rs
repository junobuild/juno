...
use crate::types::state::{MonitoringHistory, MonitoringHistoryCycles, MonitoringHistoryKey, MonitoringHistoryStable, StableState};

pub fn insert_cycles_monitoring_history(
    segment_id: &SegmentId,
    cycles: &MonitoringHistoryCycles,
)  {
    STATE.with(|state| insert_cycles_monitoring_history_impl(segment_id, cycles, &mut state.borrow_mut().stable.monitoring_history))
}
