...
use crate::monitoring::store::heap::{
    ...
    set_orbiter_strategy, set_satellite_strategy, set_ufo_strategy,
};
...
pub fn register_and_start_cycles_monitoring(
    ...
    if let Some(strategy) = &config.ufos_strategy {
        register_modules_monitoring(strategy, set_ufo_strategy)?;
    }
...
pub fn unregister_and_stop_cycles_monitoring(
    ...
    if let Some(ufo_ids) = &config.ufo_ids {
