...
use crate::http::utils::{create_cors_preflight_response, create_json_response};
...
pub fn init_certified_not_allowed_responses() {
    ...
    init_allowed_responses(&VIEW_ROUTE);
    init_allowed_responses(&VIEWS_ROUTE);
    init_allowed_responses(&EVENT_ROUTE);
    init_allowed_responses(&EVENTS_ROUTE);
    init_allowed_responses(&METRIC_ROUTE);
    init_allowed_responses(&METRICS_ROUTE);
}
...
pub fn init_allowed_responses(
    ...
    [
        ...
        .for_each(|method| {
            ...
            let mut response = create_cors_preflight_response();
