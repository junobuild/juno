...
pub async fn execute_on_post_upgrade<'js>(ctx: &Ctx<'js>) -> Result<(), JsError> {
    evaluate_loaders(ctx).await
}

async fn evaluate_loaders<'js>(ctx: &Ctx<'js>) -> Result<(), JsError> {
    let loaders_code = format!("{}", AssertSetDoc.get_loader_code());

    evaluate_async_module(ctx, HOOKS_MODULE_NAME, &loaders_code).await
}
