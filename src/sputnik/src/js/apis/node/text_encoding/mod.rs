...
use crate::js::apis::node::text_encoding::impls::Args;
use crate::js::apis::node::text_encoding::utils::{to_js_error, to_string_lossy};
...
pub fn init_text_encoding(ctx: &Ctx) -> Result<()> {
    let globals = ctx.globals();
    globals.set(
        "__javy_decodeUtf8BufferToString",
        Function::new(ctx.clone(), |cx, args| {
    ...
    opts.strict = false;
    ctx.eval_with_options::<(), _>(include_str!("./text-encoding.js"), opts)?;
