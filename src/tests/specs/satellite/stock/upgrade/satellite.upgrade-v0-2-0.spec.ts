...
describe('Satellite > Upgrade > v0.1.6 -> v0.2.0', () => {
	let pic: PocketIc;
	let actor: Actor<SatelliteActor016>;
	...
	const setControllers = async (): Promise<[Principal, SatelliteDid016.SetController][]> => {
		...
		const controllerWrite: SatelliteDid016.SetController = {
			scope: { Write: null },
			metadata: [['hello', 'world']],
			expires_at: []
		};

		const controllerAdmin: SatelliteDid016.SetController = {
			scope: { Admin: null },
			metadata: [['super', 'top']],
			expires_at: [1n]
		};
	...
	beforeEach(async () => {
		...
		const destination = await downloadSatellite('0.1.6');

		const { actor: c, canisterId: cId } = await pic.setupCanister<SatelliteActor016>({
			idlFactory: idlFactorySatellite016,
			wasm: destination,
