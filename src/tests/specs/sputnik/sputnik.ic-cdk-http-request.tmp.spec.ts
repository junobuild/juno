import type {
	AppHttpYoloRequestArgs,
	AppHttpYoloRequestResult,
...
describe('Sputnik > ic_cdk > http_request', () => {
	...
	const baseArgs: AppHttpYoloRequestArgs = {
	...
	const performHttpRequest = async ({
		...
		args: AppHttpYoloRequestArgs;
	}): Promise<AppHttpYoloRequestResult> => {
		const { app_http_yolo_request } = actor;

		const resultPromise = app_http_yolo_request(args);
		...
		await pic.mockPendingHttpsOutcall({
			...
			response: {
				type: 'success',
				body: new TextEncoder().encode(JSON.stringify(mockHttpResponse)),
				statusCode: 200,
				headers: []
			}
		});
	...
	it.only('should perform a GET request with headers', async () => {
		const args = {
			...baseArgs,
